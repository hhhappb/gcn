...
num_node_1 = 11
indices_1 = [0, 3, 5, 7, 9, 11, 13, 15, 17, 19, 20]  # 11 个部位的时候的索引。
...
num_node_2 = 5
indices_2 = [3, 5, 6, 8, 10]  # 基于indices1 继续进行索引
...
class Graph:
    def __init__(self, labeling_mode='spatial', scale=1):
        ...
        self.A_A1 = ((self.A_binary + np.eye(num_node)) / np.sum(self.A_binary + np.eye(self.A_binary.shape[0]), axis=1,
                                                                 keepdims=True))[indices_1]   # 提取这些重要部位中心点对应的边。 11 x 25
        self.A1_A2 = tools.edge2mat(neighbor_1, num_node_1) + np.eye(num_node_1)   # 基于这些部位的相联系进行进一步索引。  11 x 11
        self.A1_A2 = (self.A1_A2 / np.sum(self.A1_A2, axis=1, keepdims=True))[indices_2]   # 5 x 11
